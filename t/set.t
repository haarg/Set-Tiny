...
ok( not( $a->is_equal($b) ), to_s( $a, ' is not equal to ', $b ) );

ok( $a->is_subset($a),        to_s( $a, ' is a subset of ',     $a ) );
ok( $a->is_subset($b),        to_s( $a, ' is a subset of ',     $b ) );
ok( $b->is_subset($b),        to_s( $b, ' is a subset of ',     $b ) );
ok( not( $b->is_subset($a) ), to_s( $b, ' is not a subset of ', $a ) );

ok( $a->is_proper_subset($b), to_s( $a, ' is a proper subset of ', $b ) );
ok( not( $a->is_proper_subset($a) ),
    to_s( $a, ' is not a proper subset of ', $a ) );
ok( not( $b->is_proper_subset($b) ),
    to_s( $b, ' is not a proper subset of ', $b ) );
ok( not( $b->is_proper_subset($a) ),
    to_s( $b, ' is not a proper subset of ', $a ) );
...
ok( not( $a->is_superset($b) ), to_s( $a, ' is not a superset of ', $b ) );

ok( $b->is_proper_superset($a), to_s( $b, ' is a proper superset of ', $a ) );
ok( not( $b->is_proper_superset($b) ),
    to_s( $b, ' is not a proper superset of ', $b ) );
ok( not( $a->is_proper_superset($a) ),
    to_s( $a, ' is not a proper superset of ', $a ) );
ok( not( $a->is_proper_superset($b) ),
    to_s( $a, ' is not a proper superset of ', $b ) );

ok( $a->is_disjoint($a),        to_s( $a, ' and ', $a, ' are disjoint' ) );
ok( $a->is_disjoint($b),        to_s( $a, ' and ', $b, ' are disjoint' ) );
ok( not( $b->is_disjoint($b) ), to_s( $b, ' and ', $b, ' are not disjoint' ) );

ok(
    not( $a->is_properly_intersecting($b) ),
    to_s( $a, ' is not properly intersecting ', $b )
);
ok(
    not( $a->is_properly_intersecting($a) ),
    to_s( $a, ' is not properly intersecting ', $a )
);
ok(
    not( $b->is_properly_intersecting($b) ),
